...
根据题目要求，本应部署在 **Render.com**。但在实际部署过程中发现，Render 对新创建的用户（即使是选择 **Free** 实例）强制要求绑定信用卡进行身份验证，否则无法创建 Web Service。

由于本人没有符合条件的信用卡，因此改为将项目部署到**自己的阿里云 ECS 服务器**，并通过公网开放前端（3000 端口）和后端 API（8000 端口）。项目所有功能（股票数据获取、AI 分析、Supabase 存储）均正常运行，可通过下方链接访问。

**特此说明**：替代部署方式不影响功能完整性和在线访问，符合题目对“可访问的全栈应用”的核心要求。
